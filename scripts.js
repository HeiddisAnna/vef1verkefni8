...
const text = (() => {
  ...
  function init(_form, _items) {
    ...
    //Finnum það sem ýtt var á og sendum í rétt fall
  ...
  function formHandler(e) {
    ...
    //Hér er e form, finnum form__input
    const input = e.target.querySelector('.form__input')

    if(input.value.trim().length > 0){
      add(input.value) //Sendum það í add fallið
      //Hreynsum nú það sem stendur í rammanaum
      input.value = ''
    }
  }
  ...
  function edit(e) {
    ...
    input.value = textContent; //Sækja gamla textann
